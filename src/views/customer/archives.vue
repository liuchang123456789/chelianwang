  ...
  <div class="container">
    ...
    <div class="order_search">
      <el-form ref="ruleForm" :inline="true" :model="ruleForm" label-width="100px" class="demo-ruleForm">
        <el-form-item label="客户编号" prop="number">
          <el-input v-model="ruleForm.number" style="width: 300px;" />
        </el-form-item>
        <el-form-item label="客户全称" prop="customer_name">
          <el-input v-model="ruleForm.customer_name" style="width: 300px;" />
        </el-form-item>
        <el-form-item label="客户简称" prop="customer_name_j">
          <el-input v-model="ruleForm.customer_name_j" style="width: 300px;" />
        </el-form-item>
    ...
    <div class="tab_warpper">
      ...
      <el-table :data="tableData" border style="margin:1%;width: 98%" @selection-change="handleSelectionChange">
        ...
        <el-table-column sortable fixed prop="number" label="客户编号" width="200" />
        <el-table-column prop="customer_name" label="客户全称" width="200" />
        <el-table-column prop="customer_name_j" label="客户简称" width="150" />
        ...
        <el-table-column prop="receipt" label="线路数" width="100" />
...
export default {
  ...
  methods: {
    ...
    handleSizeChange(val) {
	  ...
	  customerUser({ page: this.page, limit: val }).then(res => {
	    this.tableData = res.result.list
	    this.total = res.result.total
	  })
    },
    handleCurrentChange(val) {
      console.log(`当前页: ${val}`)
	  customerUser({ page: val, limit: 10 }).then(res => {
	    this.tableData = res.result.list
	    this.total = res.result.total
	  })
    },
